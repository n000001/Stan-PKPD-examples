...
generated quantities {
	...
	matrix[N, nCmtPK] XPred ;
	array[nSubjects, nTheta] real <lower=0> paramsPred_ind ;
	matrix[N, nCmtPD] YPred ;
	vector[N] cHatPred ;
	vector[N] neutHatPred ;

	// individual predictions
	vector[N] cPred_ind ;
	vector[N] neutPred_ind ;
	// population predictions
	vector[N] cPred_pop ;
	vector[N] neutPred_pop ;
	...
	for(i in 1:nSubjects) {
		...
		THETAPred_ind[i][6] = THETAPred_norm[i, 6] ;
		THETAPred_ind[i][7] = THETAPred_norm[i, 7] ;

		paramsPred_ind[i] = {THETAPred_ind[i][1], 
							 THETAPred_ind[i][2], 
							 THETAPred_ind[i][3], 
							 THETAPred_ind[i][4], 
							 ka_pop, 
		...
		YPred[start[i]:end[i], ] = solvetwoCptNeut(time[start[i]:end[i]], 
													 new_amt[start[i]:end[i]], 
													 new_cmt[start[i]:end[i]], 
													 new_evid[start[i]:end[i]], 
													 paramsPred_ind[i], 
													 cHatPred[start[i]:end[i]], 
													 rdummy, idummy) ;
		
	  	neutHatPred[start[i]:end[i]] = YPred[start[i]:end[i], 5] + THETAPred_ind[i][6] ;
	}
	...
	for (t in 1:N) {
		...
		neutPred_ind[t] = lognormal_rng(log(fmax(machine_precision(), neutHat[t])), sigmaNeut) ;
		neutPred_pop[t] = lognormal_rng(log(fmax(machine_precision(), neutHatPred[t])), sigmaNeut) ;
	}
