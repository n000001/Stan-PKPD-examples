...
modelName <- "fribergKarlsson_5sub_neutro"
...
timePD <- xdata$time[iObsPD]

startPD <- rep(0, nSub) ;endPD <- rep(0, nSub)
j = 2 ; startPD[1] = 1 ; endPD[nSub] = length(timePD);
for (i in 2:length(timePD)) {
  if (timePD[i] < timePD[i-1]) { 
    startPD[j] <- i
    if (j >= 2) endPD[j-1] <- i-1
    j = j + 1
  }
}
...
data <- with(xdata,
             list(
               ...
               #added by Nina for easier implementation
               timePK = timePK,
               timePD = timePD,
               startPK = startPK,
               endPK = endPK,
...
init <- function(){
  list(CL_pop = abs(rnorm(1, 0, 20)),
       ...
       sigma = 0.2,
       mtt_pop = exp(rnorm(1, log(mttPrior), mttPriorCV)),
       circ0_pop = exp(rnorm(1, log(circ0Prior), circ0PriorCV)),
       gamma_pop = exp(rnorm(1, log(gammaPrior), gammaPriorCV)),
       alpha_pop = exp(rnorm(1, log(alphaPrior), alphaPriorCV)),
       sigmaNeut = 0.2,
       L = diag(nIIV),
